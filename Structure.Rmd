...
```{r}
map_df(subject_to_load, ~ read.delim(paste0("/Volumes/SocialDevelopmentProject/fMRI/BrainSuite/Stats/wave1/sub", .x, "-anat.roiwise.stats.txt")) %>% mutate(subject = .x)) -> combine_df
combine_df %>% 
  select(subject, ROI_ID, GM_Volume.mm.3., WM_Volume.mm.3., Mean_Thickness.mm.) %>% 
