...
myfiles.long$left <- ifelse(myfiles.long$ROI %in% c("500", "510", "520", "530"), 
                            "left", "right")
myfiles.long$region <- substr(myfiles.long$ROI, 1, 2)
myfiles.long$region <- factor(myfiles.long$region, labels = c("a", "b", "c", "d"))
...
p <- ggplot(myfiles.long, mapping = aes(y = GM_Volume.mm.3.)) +
             ...
             facet_grid(region ~ left, scales = "free")
show(p)

## TO DO LIST and OBSERVATIONS
The plot from up above produce boxplots, however the scaling is poor and needs work
